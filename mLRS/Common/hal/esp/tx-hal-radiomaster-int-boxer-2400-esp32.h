...
// UARTC = COM (CLI)
// UARTD = serial2
// UART  = JR bay pin5, full duplex CRSF serial connection to radio - code still
// calls it JR bay pin5 UARTE = in port, SBus or whatever UARTF = debug port
...
void esp_init(void) {
  // No need to configure ESP_BOOT0 which will always be IO_P0 and is pull-up by
  // default
  gpio_init(ESP_GPIO0,
            IO_MODE_OUTPUT_PP_LOW); // high -> esp will start in bootloader mode
...
const rfpower_t rfpower_list[] = {
    {.dbm = POWER_14_DBM, .mW = 25},  {.dbm = POWER_17_DBM, .mW = 50},
    {.dbm = POWER_20_DBM, .mW = 100}, {.dbm = POWER_24_DBM, .mW = 250},
    {.dbm = POWER_27_DBM, .mW = 500}, {.dbm = POWER_30_DBM, .mW = 1000},
};
