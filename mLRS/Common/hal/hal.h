...
// enter define for the board (RX_xxx, TX_xxx) into "MCU G++
// Compiler"->"Preprocessor" !!!

/* Documentation

The availability of the many features are handled via #define declarations. In
order to keep somewhat track, this naming convention is used (with few
exceptions):

- MLRS_FEATURE_XXXX: For setting different features for a target; should ONLY be
set globally/externally by the build system! Must NOT be defined in code, only
globally/externally. Note: There can be MLRS_DEV_FEATURE_XXXX flags also, but
these are for dev only.

- DEVICE_HAS_XXXX: Are set in a device's hal file to indicate the
availability/non-availability of a feature. May be defined by user in the hal
file as needed. May also be made dependent on MLRS_FEATURE_XXXX flags in hal
file.

- USE_XXXX: These are determined through some processing, which can involve the
DEVICE_HAS_XXXX flags. Must NEVER be modified or set by user.

For devs: In follow-up code therefore the USE_XXXX flags should be used (if
available) to enable/disable code for a feature. If a USE_XXXX flag is not
available (example: DEVICE_HAS_DIVERSITY) then of course the respective
DEVICE_HAS_XXXX flag needs to be used. Also, DEVICE_HAS_XXXX flags may have to
be used to distinguish the "flavor" of the feature (example: IN feature with
normal or inverted UART levels).

Many DEVICE_XXXX feature flags are available, which can be set in the device hal
files. They are listed in the following for the tx-hal and rx-hal files.

In tx-hal files:

#define DEVICE_HAS_DIVERSITY        // board supports diversity
#define DEVICE_HAS_JRPIN5           // board has a pin for JR bay Pin5/SPort
#define DEVICE_HAS_IN               // board has an IN port, which supports both
normal and inverted UART signals #define DEVICE_HAS_IN_NORMAL        // board
has an IN port, which supports only normal UART signals #define
DEVICE_HAS_IN_INVERTED      // board has an IN port, which supports only
inverted UART signals #define DEVICE_HAS_IN_ON_JRPIN5_RX  // board shares IN
with JRPin5 on RX pin, implies support of normal and inverted UART signals
#define DEVICE_HAS_IN_ON_JRPIN5_TX  // board shares IN with JRPin5 on TX pin,
implies support of normal and inverted UART signals #define
DEVICE_HAS_SERIAL_OR_COM    // board has UART (or USB) which is shared between
Serial or Com, selected by e.g. a switch #define DEVICE_HAS_NO_SERIAL        //
board has no Serial port #define DEVICE_HAS_SERIAL_ON_USB    // board has the
Serial port on native USB #define DEVICE_HAS_NO_COM           // board has no
Com port #define DEVICE_HAS_COM_ON_USB       // board has the Com port on native
USB #define DEVICE_HAS_NO_DEBUG         // board has no Debug port #define
DEVICE_HAS_DEBUG_SWUART     // implement Debug as software UART #define
DEVICE_HAS_I2C_DISPLAY          // board has a DISPLAY on I2C, and 5-way switch
#define DEVICE_HAS_I2C_DISPLAY_ROT180   // board has a DISPLAY on I2C, rotated
180°, and 5-way switch #define DEVICE_HAS_FIVEWAY          // board has 5-way
switch (without display) #define DEVICE_HAS_BUZZER           // board has a
Buzzer #define DEVICE_HAS_FAN_ONOFF        // board has a Fan, which can be set
on or off #define DEVICE_HAS_I2C_DAC          // board has a DAC for power
control on I2C #define DEVICE_HAS_SERIAL2          // board has a Serial2 port
#define DEVICE_HAS_ESP_WIFI_BRIDGE_ON_SERIAL  // board has ESP32 or ESP82xx with
RESET,GPIO support, on Serial port #define DEVICE_HAS_ESP_WIFI_BRIDGE_ON_SERIAL2
// board has ESP32 or ESP82xx with RESET,GPIO support, on Serial2 port #define
DEVICE_HAS_ESP_WIFI_BRIDGE_W_PASSTHRU_VIA_JRPIN5  // board has ESP32 or ESP82xx
with its passthrough via JRPin5 port #define
DEVICE_HAS_ESP_WIFI_BRIDGE_CONFIGURE  // board has ESP32 which allows
configuration #define DEVICE_HAS_ESP_WIFI_BRIDGE_ESP8266    // board has ESP82xx
in fact, not ESP32 #define DEVICE_HAS_ESP_WIFI_BRIDGE_BUTTON2_FLASH    // board
has button used to enter ESP flash mode #define DEVICE_HAS_HC04_MODULE_ON_SERIAL
// board has HC04 module on Serial port #define
DEVICE_HAS_HC04_MODULE_ON_SERIAL2     // board has HC04 module on Serial2 port
#define DEVICE_HAS_SYSTEMBOOT       // board has a means to invoke the system
bootloader on startup #define DEVICE_HAS_SINGLE_LED       // board has only one
LED #define DEVICE_HAS_SINGLE_LED_RGB   // board has only one LED which is RGB
WS2812, and thus can do more colors #define DEVICE_HAS_NO_LED           // board
has no LEDs at all

In rx-hal files:

#define DEVICE_HAS_DIVERSITY        // board supports diversity
#define DEVICE_HAS_DIVERSITY_SINGLE_SPI // board supports diversity on a single
SPI bus #define DEVICE_HAS_OUT              // board has an OUT port, which
supports both normal and inverted UART signals #define DEVICE_HAS_OUT_NORMAL //
board has an OUT port, which supports only normal UART signals #define
DEVICE_HAS_OUT_INVERTED     // board has an OUT port, which supports only
inverted UART signals #define DEVICE_HAS_SERIAL_OR_DEBUG  // is selected by
DEBUG_ENABLED define #define DEVICE_HAS_NO_DEBUG         // board has no Debug
port #define DEVICE_HAS_DEBUG_SWUART     // implement Debug as software UART
#define DEVICE_HAS_I2C_DAC          // board has a DAC for power control on I2C
#define DEVICE_HAS_SYSTEMBOOT       // board has a means to invoke the system
bootloader on startup #define DEVICE_HAS_SINGLE_LED       // board has only one
LED #define DEVICE_HAS_SINGLE_LED_RGB   // board has only one LED which is RGB
WS2812 #define DEVICE_HAS_FAN_ONOFF        // board has a Fan, which can be set
on or off #define DEVICE_HAS_DRONECAN         // board has a DroneCAN port

Note: Some "high-level" features are set for each device in the device_conf.h
file, and not in the device's hal file.
*/
...
//-------------------------------------------------------
// Derived Defines
// The "derived" defines digest DEVICE_HAS_XXX defines and set USE_XXX defines
// based on them.
//-------------------------------------------------------
...
#if defined DEVICE_HAS_SERIAL_OR_COM // some devices have device dependent ways
                                     // to select serial or com
#define USE_SERIAL
...
#if defined DEVICE_HAS_SERIAL2 ||                                              \
    defined DEVICE_HAS_ESP_WIFI_BRIDGE_ON_SERIAL2 ||                           \
...
#define USE_SERIAL2
    ...
    defined DEVICE_HAS_IN_INVERTED || defined DEVICE_HAS_IN_ON_JRPIN5_RX ||    \
    defined DEVICE_HAS_IN_ON_JRPIN5_TX
#define USE_IN
#endif

#if defined DEVICE_HAS_OUT || defined DEVICE_HAS_OUT_NORMAL ||                 \
    defined DEVICE_HAS_OUT_INVERTED
#define USE_OUT
#endif
...
#define USE_DISPLAY
    ...
    defined DEVICE_HAS_I2C_DISPLAY_ROT180
#define USE_I2C
...
#if defined DEVICE_HAS_HC04_MODULE_ON_SERIAL ||                                \
    defined DEVICE_HAS_HC04_MODULE_ON_SERIAL2
...
#ifdef USE_SX2
#define IF_SX(x)                                                               \
  if (Config.ReceiveUseAntenna1 || Config.TransmitUseAntenna1) {               \
    x;                                                                         \
  }
#define IF_SX2(x)                                                              \
  if (Config.ReceiveUseAntenna2 || Config.TransmitUseAntenna2) {               \
    x;                                                                         \
  }
#define IF_ANTENNA1(x)                                                         \
  if (Config.ReceiveUseAntenna1) {                                             \
    x;                                                                         \
  }
#define IF_ANTENNA2(x)                                                         \
  if (Config.ReceiveUseAntenna2) {                                             \
    x;                                                                         \
  }
...
#define SX_OR_SX2(x1, x2)                                                      \
  (Config.ReceiveUseAntenna1 || Config.TransmitUseAntenna1) ? x1 : x2
#else
#define IF_SX(x) x;
#define IF_SX2(x)
...
#if !defined DEVICE_HAS_SX128x && !defined DEVICE_HAS_SX127x &&                \
    !defined DEVICE_HAS_SX126x && !defined DEVICE_HAS_LR11xx &&                \
    !defined DEVICE_HAS_LR20xx && !defined DEVICE_HAS_DUAL_SX126x_SX128x &&    \
    !defined DEVICE_HAS_DUAL_SX126x_SX126x
#error Must be either SX128x or SX127x or SX126x or LR11xx or LR20xx !
    ...
    !defined FREQUENCY_BAND_868_MHZ && !defined FREQUENCY_BAND_866_MHZ_IN &&   \
...
// These defines don't derive from DEVICE_HAS_XXX defines, but also result in
// USE_XXX defines
//-------------------------------------------------------
...
// Empty Prototypes
//-------------------------------------------------------
// should be in the device hal files, but is just so much more convenient to
// have them here
